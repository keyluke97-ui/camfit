            ...
            <form className="space-y-8" onSubmit={handleSubmit}>
                ...
                <div className="space-y-4">
                    <label className="text-sm font-bold text-gray-700 block">
                        사진 업로드 ({files.length}/20)
                        <span className="text-gray-400 font-normal ml-2 text-xs">*최소 5장 이상 권장</span>
                    </label>

                    <div
                        className={cn(
                            "relative border-2 border-dashed rounded-xl p-8 text-center transition-all cursor-pointer min-h-[160px] flex flex-col items-center justify-center",
                            isDragOver
                    ...
                    >
                        ...
                        {files.length === 0 ? (
                            <div className="flex flex-col items-center gap-3 relative z-10 pointer-events-none">
                                <div className="p-4 bg-white rounded-full shadow-sm text-camfit-green">
                                    <UploadCloud className="w-8 h-8" />
                                </div>
                                <div>
                                    <p className="font-semibold text-gray-900">클릭하거나 사진을 드래그하세요</p>
                                    <p className="text-xs text-gray-500 mt-1">최대 20장까지 한 번에 선택 가능</p>
                                </div>
                            </div>
                        ) : (
                            <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 w-full pt-2" onClick={(e) => e.stopPropagation()}>
                                ...
                                <button
                                    type="button"
                                    onClick={() => fileInputRef.current?.click()}
                                    className="aspect-square flex flex-col items-center justify-center border border-gray-200 rounded-lg bg-gray-50 hover:bg-gray-100 text-gray-500 text-xs gap-1"
                                >
                                ...
                                {files.map((file, idx) => (
                                    <div key={idx} className="relative aspect-square rounded-lg overflow-hidden group shadow-sm bg-gray-100">
                                        {/* Create object URL for preview - in real app handle cleanup, here simplified */}
                                        <img
                ...
                <Button
                    className="w-full h-14 text-lg font-bold shadow-camfit-green/20 hover:shadow-camfit-green/40 rounded-xl"
                    type="submit"
