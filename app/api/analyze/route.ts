...
1. **Hygiene (청결) -> "보수적 평가 (Worst Case)"**
   ...
   - **[Scoring Cap]** 화장실/개수대/샤워실 중 단 하나의 사진도 식별되지 않는다면(검증 불가), **위생 점수를 최대 60점으로 제한(Cap)**하세요.
...
- Target 100: 모든 시설(화장실, 샤워실, 온천, 매점 등) 바닥에 물기가 없고, 물건이 완벽하게 정돈되어 있으며, 노후화 흔적이 전혀 없음.
- Target 40: 전반적으로 위생적이나, 매점의 물건 정리가 어수선하거나 관리동 입구에 잡풀이 보이는 등 세밀한 관리가 부족함.
- Target 60 (Cap): 주요 위생 시설(화장실/샤워실/개수대) 사진이 아예 없어 AI가 직접 검증할 수 없는 경우.
